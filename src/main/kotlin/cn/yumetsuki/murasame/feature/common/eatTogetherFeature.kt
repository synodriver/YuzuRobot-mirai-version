...
fun GroupMessageSubscribersBuilder.eatTogether() {
        ...
        return if (now.isAfter(startTime) && now.isBefore(endTime)) {
            goHanRecordDao.queryGoHanRecordBetweenTime(
                ...
                qqUserDao.findQQUserByUserIdAndGroupIdOrNewDefault(
                    ...
                    "唔姆～今天也和主人一起开心地吃早饭了呢～(好感度+$favoriteAdd, 当前好感度: ${qqUser.favorite}"
                }
            }
        } else {
            "(唔....现在已经不是吃早饭的时间了呢, 早饭时间: ${startTime.simpleFormat()}～${endTime.simpleFormat()}"
        }
    }
